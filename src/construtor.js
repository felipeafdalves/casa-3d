...
export class Construtor {
  ...
  construirParede(pav, w, nivel, topo, g) {
    ...
    const peca = (t0, t1, h0, h1) => {
      // troço com altura limitada (ex.: verga/pilarete debaixo de um lance de escada)
      const pm = ponto((t0 + t1) / 2);
      for (const h of pav.alturasParede || []) if (Math.hypot(pm[0] - h.perto[0], pm[1] - h.perto[1]) < 0.3) h1 = Math.min(h1, h.topo);
  ...
  construirEscada(esc) {
    ...
    for (const gd of esc.guardas || []) {
      // `lance: i` segue a rampa do lance i (índice em `lances`); 'rampa' segue o lance mais alto no ponto; z fixo caso contrário
      const base = gd.lance !== undefined ? (x, y) => alturas[gd.lance](x, y) ?? alturaEscada(x, y) ?? gd.z ?? 0 : gd.z === 'rampa' ? (x, y) => alturaEscada(x, y) ?? 0 : () => gd.z;
      this.construirGuarda(gd.a, gd.b, gd.altura, base, g, gd.estilo);
    }
  }
  ...
  construirExterior() {
    ...
    this.construirEscada({
      id: 'escada-entrada',
      material: 'porcelanato_cinza',
      materialEspelho: 'cimento_parede',
      lances: [{ de: [7.0, -6.83], para: [7.0, -4.43], largura: 2.4, z0: 0, z1: 1.45, degraus: 8 }],
      // corrimãos metálicos pretos dos dois lados, a acompanhar os degraus
      guardas: [
        { a: [5.86, -6.83], b: [5.86, -4.43], altura: 0.95, lance: 0, estilo: 'preto' },
        { a: [8.14, -6.83], b: [8.14, -4.43], altura: 0.95, lance: 0, estilo: 'preto' },
      ],
    });
