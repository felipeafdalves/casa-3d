...
const pecas = {
  ...
  lareira({ altura = 2.8, largura = 1.9, bancos = true }) {
    // coluna de pedra até ao teto, boca da lareira, viga-consola de madeira e TV; bancos de janela nos lados
    const g = new THREE.Group();
    g.add(caixa(largura, altura, 0.55, 'pedra', 0, altura / 2, 0));
    g.add(caixa(1.1, 0.55, 0.3, new THREE.MeshStandardMaterial({ color: '#141210', roughness: 1 }), 0, 0.62, 0.18));
    g.add(caixa(1.1, 0.04, 0.04, 'metal_preto', 0, 0.9, 0.29));
    for (let i = 0; i < 3; i++) g.add(cilindro(0.05, 0.6, 'nogueira', -0.25 + i * 0.25, 0.42, 0.15, 8).rotateX(Math.PI / 2));
    const lume = new THREE.Mesh(new THREE.PlaneGeometry(0.7, 0.35), new THREE.MeshStandardMaterial({ color: '#ff7a1c', emissive: '#ff8a2a', emissiveIntensity: 3, transparent: true, opacity: 0.85 }));
    lume.position.set(0, 0.55, 0.32);
    g.add(lume);
    const luz = new THREE.PointLight('#ff9a3c', 4, 5, 2);
    luz.position.set(0, 0.7, 0.6);
    g.add(luz);
    g.add(caixa(1.5, 0.12, 0.28, 'nogueira', 0, 1.25, 0.34));
    g.add(caixa(1.45, 0.82, 0.04, 'tela_tv', 0, 1.95, 0.29));
    g.add(cilindro(0.05, 0.16, 'ceramica_branca', 0.55, 1.39, 0.36, 10, 0.04));
    g.add(planta(0.55, 1.4, 0.36, 0.07, 4));
    const colisores = [col(largura / 2, 0.28, altura)];
    if (bancos) {
      for (const s of [-1, 1]) {
  ...
  estante_arcos({ largura = 3.6, altura = 2.7 }) {
    ...
    const n = 3;
    const lw = largura / n;
    g.add(caixa(largura, altura, 0.4, 'marcenaria_branca', 0, altura / 2, 0));
    // armário inferior com portas shaker e tampo de madeira
    for (let i = 0; i < n * 2; i++) g.add(shaker(lw / 2 - 0.04, 0.72, 'marcenaria_branca', -largura / 2 + (lw / 2) * (i + 0.5), 0.42, 0.21, 'bronze'));
    g.add(caixa(largura + 0.02, 0.04, 0.44, 'nogueira', 0, 0.82, 0.01));
    // nichos em arco com fundo ripado e prateleiras de madeira
    for (let i = 0; i < n; i++) {
      const x = -largura / 2 + lw * (i + 0.5);
      const r = lw / 2 - 0.12;
      const hArco = altura - 0.9 - 0.12;
      const forma = new THREE.Shape();
      forma.moveTo(-r, 0);
      forma.lineTo(-r, hArco - r);
      forma.absarc(0, hArco - r, r, Math.PI, 0, true);
      forma.lineTo(r, 0);
      forma.lineTo(-r, 0);
      const nicho = new THREE.Mesh(new THREE.ExtrudeGeometry(forma, { depth: 0.3, bevelEnabled: false }), material('shiplap_branco'));
      nicho.position.set(x, 0.86, -0.1);
      nicho.receiveShadow = true;
      g.add(nicho);
      // moldura branca à volta do arco (ligeiro relevo)
      const anel = new THREE.Mesh(new THREE.TorusGeometry(r + 0.03, 0.03, 8, 24, Math.PI), material('marcenaria_branca'));
      anel.position.set(x, 0.86 + hArco - r, 0.2);
      g.add(anel);
      for (let k = 0; k < 3; k++) g.add(caixa(2 * r - 0.02, 0.03, 0.28, 'nogueira', x, 1.2 + k * 0.42, 0.05));
      g.add(livros(x - r + 0.08, 1.23, 0.02, 6));
      if (i === 1) g.add(pecas.quadro({ largura: 0.5, altura: 0.4, cor: '#7f8b6a', moldura: 'nogueira' }).grupo.translateX(x).translateY(1.95).translateZ(0.06));
      else g.add(cilindro(0.06, 0.16, 'ceramica_branca', x + 0.15, 1.72, 0.08, 10, 0.045));
      g.add(livros(x - r + 0.1, 2.06, 0.02, 4));
    }
  ...
  carro({ cor = 'carro_preto', modelo = 'suv' }) {
    const g = new THREE.Group();
    const picape = modelo === 'picape';
    const L = picape ? 5.3 : 4.75;
    const W = 1.92;
    const h = L / 2;
    const pintura = M(cor);
    ...
    const perfil = picape
      ? [[-h, 0.42], [-h - 0.02, 0.98], [-h + 0.08, 1.1], [-0.25, 1.1], [-0.2, 1.74], [0.0, 1.84], [0.62, 1.84], [1.25, 1.34], [1.5, 1.14], [h - 0.25, 1.06], [h + 0.02, 0.78], [h, 0.42]]
      : [[-h, 0.4], [-h - 0.03, 0.9], [-h + 0.05, 1.12], [-h + 0.32, 1.7], [-h + 0.8, 1.78], [0.35, 1.78], [1.05, 1.28], [1.35, 1.08], [h - 0.15, 1.0], [h + 0.02, 0.72], [h, 0.4]];
    const forma = new THREE.Shape();
    ...
    g.add(corpo);
    // vidros laterais (ligeiramente salientes), com montante entre portas
    const zVidro0 = picape ? -0.15 : -h + 0.5;
    const zVidro1 = picape ? 1.15 : 1.0;
    for (const sx of [-1, 1]) {
      const v = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.5, zVidro1 - zVidro0), vidro);
      v.position.set(sx * (W / 2 - 0.0), 1.4, (zVidro0 + zVidro1) / 2);
      g.add(v);
      g.add(caixa(0.03, 0.52, 0.05, preto, sx * (W / 2 + 0.005), 1.4, picape ? 0.55 : (zVidro0 + zVidro1) / 2));
      // puxadores e retrovisores
      for (const zp of picape ? [0.35] : [-0.3, 0.75]) g.add(caixa(0.03, 0.03, 0.18, cromado, sx * (W / 2 + 0.01), 1.0, zp));
      g.add(caixa(0.2, 0.1, 0.16, pintura, sx * (W / 2 + 0.1), 1.2, zVidro1 - 0.1));
    }
    // para-brisas e óculo traseiro (painéis inclinados)
    const pb = new THREE.Mesh(new THREE.BoxGeometry(W - 0.3, 0.02, 0.86), vidro);
    pb.position.set(0, 1.55, picape ? 0.95 : 0.7);
    pb.rotation.x = picape ? -0.66 : -0.62;
    g.add(pb);
    const ot = new THREE.Mesh(new THREE.BoxGeometry(W - 0.4, 0.02, picape ? 0.5 : 0.62), vidro);
    ot.position.set(0, picape ? 1.5 : 1.48, picape ? -0.22 : -h + 0.19);
    ot.rotation.x = picape ? 1.45 : 1.15;
    g.add(ot);
    ...
    const zRodas = picape ? [-1.75, 1.55] : [-1.45, 1.45];
    for (const z of zRodas) {
      const cava = new THREE.Mesh(new THREE.CylinderGeometry(0.47, 0.47, W + 0.02, 24), preto);
      cava.rotation.z = Math.PI / 2;
      cava.position.set(0, 0.42, z);
      g.add(cava);
      for (const sx of [-1, 1]) {
        const r = new THREE.Mesh(new THREE.CylinderGeometry(0.37, 0.37, 0.26, 24), pneu);
        r.rotation.z = Math.PI / 2;
        r.position.set(sx * (W / 2 - 0.16), 0.37, z);
        r.castShadow = true;
        g.add(r);
        const jante = cilindro(0.25, 0.27, cromado, sx * (W / 2 - 0.16), 0.37, z, 10);
        jante.rotation.z = Math.PI / 2;
        g.add(jante);
        for (let k = 0; k < 5; k++) {
          const raio = caixa(0.03, 0.2, 0.04, preto, sx * (W / 2 - 0.02), 0.37, z);
    ...
    g.add(caixa(0.9, 0.3, 0.04, preto, 0, 0.78, h + 0.03));
    g.add(caixa(W - 0.2, 0.18, 0.08, preto, 0, 0.5, h + 0.02));
    ...
    for (const sx of [-1, 1]) {
      g.add(caixa(0.42, 0.14, 0.04, farol, sx * (W / 2 - 0.35), 0.86, h + 0.03));
      g.add(caixa(0.36, 0.14, 0.04, farolim, sx * (W / 2 - 0.3), 0.85, -h - 0.04));
    }
    g.add(caixa(0.4, 0.13, 0.02, material('marcenaria_branca'), 0, 0.6, -h - 0.05));
    // barras de tejadilho (SUV) ou capota marítima (picape)
    if (picape) g.add(caixa(W - 0.3, 0.03, h - 0.3, preto, 0, 1.12, -h / 2 - 0.12));
    else for (const sx of [-1, 1]) g.add(caixa(0.05, 0.06, 1.9, preto, sx * 0.62, 1.82, -0.3));
